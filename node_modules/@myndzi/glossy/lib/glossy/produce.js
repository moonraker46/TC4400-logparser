...
GlossyProducer.prototype.produce = function(options, callback) {
    // fill in a timestamp by default. options.date takes precedence over options.time
    // invalid inputs will be rendered as the nil value

    // options.date takes precedence over options.time
    var inputDate = options.date || options.time;

    // unspecified = current timestamp
    if ((inputDate == null)) {
        inputDate = new Date();
    }
    ...
    if(this.type == 'RFC5424') {
        ...
        msgData.push(prival);
        msgData.push(inputDate ? generateDate(inputDate) : '-');
        ...
        options.timestamp = inputDate ? generateBSDDate(inputDate) : '-';
        msgData.push(
            calculatePrival({
                facility: options.facility,
                severity: options.severity
